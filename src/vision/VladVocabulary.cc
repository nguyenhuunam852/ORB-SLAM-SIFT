#include "vision/VladVocabulary.h"
...
{
    cv::Mat vladVec = cv::Mat::zeros(mCentroids.rows, mCentroids.cols, CV_32F);

    for (int r = 0; r < descriptors.rows; ++r) {
        const cv::Mat row = descriptors.row(r);
        const int c = nearestCentroid(row);
        vladVec.row(c) += (row - mCentroids.row(c));
    }

    for (int c = 0; c < vladVec.rows; ++c) {
        cv::Mat centroidResidual = vladVec.row(c);
        const double n = cv::norm(centroidResidual, cv::NORM_L2);
        if (n > std::numeric_limits<float>::epsilon())
            centroidResidual /= n;
    }

    cv::Mat flat = vladVec.reshape(1, 1).clone();

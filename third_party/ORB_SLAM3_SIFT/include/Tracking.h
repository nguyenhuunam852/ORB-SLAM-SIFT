...
// Viewer.h here is a minimal STUB (see its own doc comment) -- Tracking.cc
// actually calls methods on mpViewer (guarded by `if(mpViewer)`, always
// false since mpViewer is permanently null in this vendored, headless-only
// copy), so it needs the complete type, unlike System.h/Atlas.h which only
// ever store the pointer.
#include "Viewer.h"

    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    // Viewer intentionally excluded from this vendored copy (see
    // CMakeLists.txt's own doc comment on the orbslam3_kitti_ate target --
    // this is a headless batch tool, Viewer.cc/.h were never copied in to
    // avoid the Pangolin/GL dependency entirely). bUseViewer must be false
    // for every caller in this project; mpViewer stays permanently null.
    if (bUseViewer) {
        throw std::runtime_error("bUseViewer=true is not supported by this vendored ORB-SLAM3 copy "
                                  "(Viewer.cc/.h were deliberately not vendored -- see System.cc)");
    }
